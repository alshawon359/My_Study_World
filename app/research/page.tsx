  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
        <div className="mb-4 flex items-center justify-between"><div><p className="font-semibold">Research analysis</p><p className="text-xs text-slate-400">Capture the parts you will want when writing later.</p></div><Sparkles className="h-4 w-4 text-cyan-300" /></div>
        <div className="grid gap-4 md:grid-cols-2">
      ...
      <div className="rounded-2xl border border-dashed border-cyan-400/30 bg-cyan-400/[0.04] p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Materials</p><p className="text-xs text-slate-400">PDF, PPTX, images and notes up to 5 MB each.</p></div><label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-cyan-400 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-300"><Upload className="h-4 w-4" /> Add files<input type="file" className="hidden" accept=".pdf,.ppt,.pptx,image/*" multiple onChange={onUpload} /></label></div>
        {formData.materials.length > 0 && <div className="mt-4 space-y-2">{formData.materials.map((material) => <div key={material.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-slate-950/40 px-3 py-2"><div className="flex min-w-0 items-center gap-2"><MaterialIcon type={material.type} /><span className="truncate text-sm">{material.name}</span><span className="text-xs text-slate-500">{(material.size / 1024 / 1024).toFixed(1)} MB</span></div><button type="button" onClick={() => onRemoveMaterial(material.id)} className="rounded p-1 text-slate-400 hover:bg-white/10 hover:text-white" aria-label={`Remove ${material.name}`}><X className="h-4 w-4" /></button></div>)}</div>}
      </div>
...
export default function ResearchPage() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-slate-950 text-slate-300"><Loader2 className="mr-3 h-6 w-6 animate-spin text-cyan-300" />Loading your research library...</div>;

  return <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950 p-4 text-white sm:p-6"><div className="mx-auto max-w-7xl">
    <section className="mb-7 rounded-3xl border border-white/10 bg-white/[0.06] p-6 shadow-2xl shadow-indigo-950/30 backdrop-blur-md md:p-8"><div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.22em] text-cyan-300"><Sparkles className="h-4 w-4" /> Research library</div><div className="flex items-center gap-3"><div className="rounded-2xl bg-gradient-to-br from-cyan-300 to-fuchsia-400 p-3"><BookOpen className="h-8 w-8 text-slate-950" /></div><div><h1 className="text-3xl font-bold tracking-tight md:text-4xl">Research Papers</h1><p className="mt-1 text-sm text-slate-300 md:text-base">A focused home for your literature, insights, and evidence.</p></div></div></div><Dialog open={dialogMode === 'add'} onOpenChange={(open) => { if (!open) setDialogMode(null); }}><DialogTrigger asChild><Button onClick={() => { setFormData(emptyForm); setDialogMode('add'); }} className="h-11 rounded-xl bg-cyan-300 px-5 font-semibold text-slate-950 hover:bg-cyan-200"><Plus className="mr-2 h-4 w-4" />Add paper</Button></DialogTrigger><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto border-slate-700 bg-slate-900 text-white"><DialogHeader><DialogTitle className="text-xl">Add to your research library</DialogTitle></DialogHeader><PaperEditor formData={formData} setFormData={setFormData} onUpload={handleUpload} onRemoveMaterial={removeMaterial} onSave={savePaper} onCancel={() => setDialogMode(null)} isSaving={isSaving} saveLabel="Save paper" /></DialogContent></Dialog></div><div className="mt-7 flex flex-wrap items-center gap-3 text-sm text-slate-300"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-emerald-300"><Database className="h-4 w-4" />Database connected</span><span className="text-slate-500">•</span><span>Synced across devices</span></div></section>
    <div className="mb-7 grid grid-cols-2 gap-3 md:grid-cols-4">{metrics.map(({ label, value, icon: Icon, color }) => <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.06] p-4"><div className="flex items-center justify-between"><span className="text-xs font-medium uppercase tracking-wider text-slate-400">{label}</span><Icon className={`h-4 w-4 ${color}`} /></div><div className="mt-3 text-3xl font-bold">{value}</div></div>)}</div>
    <div className="mb-7 grid gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-3 md:grid-cols-[1fr_220px]"><div className="relative"><Search className="absolute left-3 top-3 h-5 w-5 text-slate-500" /><Input placeholder="Search title, author, or topic..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-11 border-white/10 bg-white/[0.08] pl-10 text-white placeholder:text-slate-500" /></div><Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="h-11 border-white/10 bg-white/[0.08] text-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ALL">All statuses</SelectItem>{STATUS_OPTIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></div>
    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredPapers.length === 0 ? <div className="col-span-full rounded-3xl border border-dashed border-white/15 bg-white/[0.04] py-16 text-center"><FileText className="mx-auto mb-4 h-12 w-12 text-slate-600" /><p className="text-lg font-semibold text-slate-200">No papers match this view</p><p className="mt-1 text-sm text-slate-500">Add a paper or adjust your search to continue.</p></div> : filteredPapers.map((paper) => { const status = statusLabel(paper.status); const materials = parseMaterials(paper.materials); return <Card key={paper.id} onClick={() => openPaper(paper)} className="group cursor-pointer rounded-2xl border-white/10 bg-white/[0.06] shadow-xl shadow-slate-950/20 transition-all hover:-translate-y-1 hover:border-cyan-400/50 hover:bg-white/[0.09]"><CardHeader><div className="flex items-start justify-between gap-3"><div className="min-w-0"><CardTitle className="line-clamp-2 text-lg text-white">{paper.title}</CardTitle><p className="mt-1 line-clamp-1 text-sm text-slate-400">{paper.authors}</p></div>{status && <Badge className={`shrink-0 border-0 ${status.color}`}>{status.label}</Badge>}</div></CardHeader><CardContent><div className="space-y-3 text-sm"><div className="flex flex-wrap gap-2 text-slate-400">{paper.year && <span>{paper.year}</span>}{paper.topic && <span className="rounded-full bg-white/10 px-2 py-0.5">{paper.topic}</span>}{materials.length > 0 && <span className="inline-flex items-center gap-1 text-cyan-300"><File className="h-3 w-3" />{materials.length} material{materials.length > 1 ? 's' : ''}</span>}</div>{paper.researchProblem && <p className="line-clamp-2 text-slate-300">{paper.researchProblem}</p>}<div className="flex items-center justify-between border-t border-white/10 pt-3 text-xs text-slate-500"><span>Open paper details</span><Eye className="h-4 w-4 text-cyan-300" /></div></div></CardContent></Card>; })}</div>

    <Dialog open={dialogMode === 'view' || dialogMode === 'edit'} onOpenChange={(open) => { if (!open) { setDialogMode(null); setSelectedPaper(null); } }}><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto border-slate-700 bg-slate-900 text-white"><DialogHeader><div className="flex items-center justify-between"><DialogTitle className="text-xl">{dialogMode === 'edit' ? 'Edit research paper' : 'Paper details'}</DialogTitle>{dialogMode === 'view' && <Button variant="ghost" size="sm" onClick={() => setDialogMode('edit')}><Edit className="mr-2 h-4 w-4" />Edit</Button>}</div></DialogHeader>{dialogMode === 'edit' ? <PaperEditor formData={formData} setFormData={setFormData} onUpload={handleUpload} onRemoveMaterial={removeMaterial} onSave={savePaper} onCancel={() => setDialogMode('view')} isSaving={isSaving} saveLabel="Update paper" /> : selectedPaper && <div className="space-y-5"><div><div className="mb-2 flex flex-wrap gap-2">{statusLabel(selectedPaper.status) && <Badge className={`border-0 ${statusLabel(selectedPaper.status)?.color}`}>{statusLabel(selectedPaper.status)?.label}</Badge>}{selectedPaper.year && <span className="text-sm text-slate-400">{selectedPaper.year}</span>}</div><h2 className="text-2xl font-bold">{selectedPaper.title}</h2><p className="mt-1 text-slate-400">{selectedPaper.authors}</p></div>{selectedPaper.link && <a href={selectedPaper.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200"><ExternalLink className="h-4 w-4" />Open source paper</a>}<div className="grid gap-4 sm:grid-cols-2">{[['Research problem', selectedPaper.researchProblem], ['Dataset', selectedPaper.dataset], ['Method', selectedPaper.method], ['Model', selectedPaper.model], ['Key results', selectedPaper.results], ['Limitations', selectedPaper.limitations], ['Important notes', selectedPaper.importantNotes], ['My thoughts', selectedPaper.myThoughts], ['Research ideas', selectedPaper.researchIdeas]].filter(([, value]) => value).map(([label, value]) => <div key={String(label)} className="rounded-xl border border-white/10 bg-white/[0.04] p-3 sm:col-span-2"><p className="mb-1 text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</p><p className="whitespace-pre-wrap text-sm leading-6 text-slate-200">{value}</p></div>)}</div>{parseMaterials(selectedPaper.materials).length > 0 && <div><p className="mb-2 text-sm font-semibold">Materials</p><div className="space-y-2">{parseMaterials(selectedPaper.materials).map((material) => <a key={material.id} href={material.dataUrl} download={material.name} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 hover:bg-white/10"><span className="flex min-w-0 items-center gap-2"><MaterialIcon type={material.type} /><span className="truncate text-sm">{material.name}</span></span><Download className="h-4 w-4 text-cyan-300" /></a>)}</div></div>}<Button variant="destructive" onClick={() => deletePaper(selectedPaper.id)}><Trash2 className="mr-2 h-4 w-4" />Delete paper</Button></div>}</DialogContent></Dialog>
  </div></div>;
}
